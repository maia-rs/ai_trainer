...
class ExercicioRepositorio:
    ...
    def obter_exercicio_por_id_externo(self, id_externo: int) -> Exercicio | None:
    ...
    def buscar_exercicios(
        ...
        stmt = select(Exercicio).where(
            Exercicio.status == StatusExercicio.ATIVO.value
        )

        if nome:
            stmt = stmt.where(
                Exercicio.nome.ilike(f"%{nome}%")
            )
        ...
        if grupo_muscular:
            stmt = stmt.where(
                Exercicio.grupo_muscular.ilike(f"%{grupo_muscular}%")
            )
